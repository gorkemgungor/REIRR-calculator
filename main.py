...
class MainWindow(QMainWindow):
    ...
    def export_pdf(self):
        ...
        elements.append(Paragraph("LQ Model Calculator — Radiotherapy Dose Planner", title_style))
        elements.append(Spacer(1, 0.3*cm))

        pt_data = [["Patient:", p["name"], "ID:", p["id"]],
                   ["DOB:", p["dob"], "Plan Date:", p["plan_date"]],
                   ["Diagnosis:", p["diagnosis"], "Physician:", p["physician"]],
                   ["Notes:", p["notes"], "", ""]]
        ...
        # Courses
        ch = ["", "Dose/Fr (Gy)", "# Fr", "Total (Gy)", "EQD2 (α/β=3)", "BED (α/β=3)",
              "EQD2 (α/β=10)", "BED (α/β=10)", "EQD2 (manual)", "BED (manual)"]
        cr = [ch]
        for lbl, c in [("COURSE 1", self.c1), ("COURSE 2", self.c2), ("COURSE 3", self.c3)]:
            ...
            cr.append([lbl, f"{d:.2f}", f"{n:.2f}", f"{d*n:.2f}",
                ...
                f"{c.get_eqd2_manual():.2f}", f"{calc_bed(d,n,ab):.2f}"])
        ...
        ct = Table(cr, colWidths=[2.2*cm]+[2.4*cm]*9)
        ...
        oar_hdr = ["Frz", "OAR", "α/β", "Limit\n(EQD2)", "C1 Red%", "C2 Red%",
                   "Allowed\nEQD2", "C1 EQD2", "C2 EQD2", "C3 EQD2", "SUM", "Status"]
        oar_rows = [oar_hdr]
        for row, oar in enumerate(OAR_DATA):
        ...
        if len(oar_rows) > 1:
            ow = [1.0*cm, 3.8*cm, 1.2*cm, 1.6*cm, 1.2*cm, 1.2*cm, 1.8*cm, 1.6*cm, 1.6*cm, 1.6*cm, 1.6*cm, 2.2*cm]
    ...
    def export_excel(self):
        ...
        ws = wb.active; ws.title = "LQ Calculator"
        ...
        row += 2
        for lbl, val in [("Patient",p["name"]),("ID",p["id"]),("DOB",p["dob"]),
                         ("Plan Date",p["plan_date"]),("Diagnosis",p["diagnosis"]),
